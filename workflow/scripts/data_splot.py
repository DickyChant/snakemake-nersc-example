...
def load(coll, pd, mlo, mhi, max_files, max_cands):
    ...
    max_cands candidates are collected."""
    ...
    for f in fs[:cap_files]:
        ...
        cols = d.AsNumpy([f"{coll}_m_ditrack", f"{coll}_fitted_mass"])
        used += 1
        mcol, zcol = cols[f"{coll}_m_ditrack"], cols[f"{coll}_fitted_mass"]
        if len(mcol) == 0:
            continue
...
def main():
    ...
    m, mz, used, total = load(args.collection, cfg["pd"], cfg["mlo"], cfg["mhi"],
                              args.max_files, args.max_cands)
    print(f"[{args.collection}] data files {used}/{total}"
    ...
    draw_fit(args.collection, mtt, data, model, sig, bkg, nsig, nbkg, cfg)
    ...
    hR, hS, hB = draw_splot(args.collection, mz, sw_sig, sw_bkg)

    fo = ROOT.TFile(f"{OUT_DIR}/{args.collection}_splot.root", "RECREATE")
    for h in (hR, hS, hB): h.Write()
    fo.Close()
    print(f"  wrote {OUT_DIR}/{args.collection}_{{ditrack_fit,splot_Zsignal,splot_panels}}.png (+ .root)")
